...
class BaseCoordinator: Coordinator {
    ...
    func moveToMoviesList() {
        ...
        let movieViewViewModel = MoviesListViewModel(model: repository) { [weak self] action in
            switch action {
            case .pushDetailedViewController(let indexPath, let movies):
                self?.moveToDetailedPage(movies: movies, indexPath: indexPath)
            }
        }
        let movieViewController = MovieViewController(viewModel: movieViewViewModel)
        movieViewController.coordinator = self
        navigationController.pushViewController(movieViewController)
    }
    
    func moveToDetailedPage(movies: [MovieData], indexPath: IndexPath) {
        let viewModel = DetailedMovieViewModel(movies: movies, indexPath: indexPath)
        let viewController = DetailedMovieViewController(viewModel: viewModel)
        viewController.coordinator = self
        navigationController.presentMediumPageSheet(viewController)
    }
